...
class MFC_aileron_Env(gym.Env):
    ...
    def __init__(self, goal_position=0, steps_per_ep = 200):
        ...
        self.actions = np.array([-6, -2, -1, 0, 1, 2, 6])
    ...
    def get_reward(self, error):
        ...
        if self.goal_position>self.last_goal:
            if error < -0.005:
                return -(error*error)*10
            else: 
                return -(error*error)
        elif self.goal_position<self.last_goal:
            if error > 0.005:
                return -(error*error)*10
            else:
                return -(error*error)
    ...
    def reset(self, goal=0):
        ...
        self.last_goal = self.state[0,-1,0]
        self.num_steps = 0
    ...
    def next_goal(self, goal):
        self.last_goal = self.goal_position
        self.goal_position = goal
