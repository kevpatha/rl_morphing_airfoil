...
class MFC_aileron_Env(gym.Env):
    ...
    def __init__(self, start_volt = 50, goal_position=0, steps_per_ep = 200):
        ...
        #Using Linear Model
        self.flex_to_laz = LinearModel(input_dim=1)
        self.flex_to_laz.load_state_dict(T.load('flex_and_laz_models/simple_flex_to_laz_linear_model_aug18_data9.hdf'))
        ...
        self.act_mean = 50
        self.act_std = 26.8
        self.pos_mean = 750 #755.526 #meausered by flex
        self.pos_std = 70 #76.0765 #measured by flex
        self.laz_mean = 497.5
        self.laz_std = 226
        ...
        self.actions = np.array([-6,-2,-1,0,1,2,6])#np.array([-10,-6,-2,0,2,6,10])
    ...
    def step(self, action):
        ...
        self.new_volt = self.denormalize_volt(self.old_volt) + self.actions[action]
        ...
        new_pos = self.flex_to_laz(T.tensor(norm_flex.reshape((1,-1)), dtype=T.float).to(self.flex_to_laz.device)).detach().cpu().numpy()
